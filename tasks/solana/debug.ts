    ...
    .setAction(async (taskArgs: DebugTaskArgs) => {
        ...
        try {
            ...
            if (!programAccount?.data) {
                ...
                const hasAuthority = programDataAccount?.data && programDataAccount.data.length >= 13 && programDataAccount.data[12] === 1
                if (hasAuthority && programDataAccount.data.length >= 45) {
                    programUpgradeAuthority = new PublicKey(programDataAccount.data.slice(13, 45))
                } else {
                    programUpgradeAuthority = null
                }
            }
        } catch (e) {
            // Program might not be upgradeable or account might not exist
            console.warn(`Could not fetch program upgrade authority: ${e}`)
            programUpgradeAuthority = null
        }
        ...
        const printOftStore = async () => {
            ...
            DebugLogger.keyValue('OFT Program Upgrade Authority', programUpgradeAuthority?.toBase58() ?? 'None (immutable)')
            DebugLogger.keyValue('OFT Type', oft.types.OFTType[oftStoreInfo.oftType])
            DebugLogger.keyValue('OFT Admin', oftStoreInfo.admin)
            ...
            DebugLogger.keyValue('Pauser', unwrapOption(oftStoreInfo.pauser, () => 'None'))
            DebugLogger.keyValue('Unpauser', unwrapOption(oftStoreInfo.unpauser, () => 'None'))
        ...
        const printToken = async () => {
            ...
            DebugLogger.keyValue('Token URI', tokenMetadata?.uri ?? 'N/A')
            ...
            DebugLogger.keyValue('Metadata Update Authority', tokenMetadata?.updateAuthority ?? 'N/A (no Metaplex Metadata)')
