import { TOKEN_2022_PROGRAM_ID, createPauseInstruction } from '@solana/spl-token'
...
task('lz:solana:pause', 'Pauses a token')
    .addParam('eid', 'Solana mainnet (30168) or testnet (40168) eid', undefined, devtoolsTypes.eid, false)
    .addParam('tokenAddr', 'The mint address of the token to pause', undefined, devtoolsTypes.string, false)
    .addParam('squadsAddr', 'The squads address to use for the pause', undefined, devtoolsTypes.string, false)
    ...
    .setAction(async ({ eid, tokenAddr, squadsAddr, signerAddr }: TxTaskArgs) => {
        console.log(`Pausing token ${tokenAddr}...`)
