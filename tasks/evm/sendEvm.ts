...
export async function sendEvm(
    ...
    // 2️⃣ load OFT ABI
    const oftArtifact = await srcEidHre.artifacts.readArtifact('WXRPMintBurnOFTAdapter')
    const oft = await srcEidHre.ethers.getContractAt(oftArtifact.abi, wrapperAddress) // signer removed

    // 3️⃣ fetch the underlying ERC-20
    const underlying = await oft.token()
    // 4️⃣ fetch decimals from the underlying token
    const erc20 = await srcEidHre.ethers.getContractAt('WXRPToken', underlying) // signer removed
